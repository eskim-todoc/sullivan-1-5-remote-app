...
public class MainActivity extends AppCompatActivity
{
      ...
      public void initMainActivity()
      {
            ...
            // 앱 잠금 비밀번호 버튼에 대한 롱클릭 리스너 등록 -> 로깅 데이터 출력 화면으로 접근하기 위한 이스터에그 구현
            initNumberLongClick(); // del, del, 1, 4, del, del, 0, 7
    ...
    */
      ...
      public void initAllForApp()
      {
            ...
            for (Device device : devices)
            {
                  ...
                  writeMessage(LoggingUtils.LOGGING_DEVICE_REMOVED,
                              "name=" + device.getDeviceName()
                                          + ", addr=" + device.getDeviceMacAddress()
                                          + ", serial=" + device.getDeviceSerial()
                                          + ", user=" + device.getImplantUserName()); // Logging
            }
            ...
            // 4. 앱 비밀번호 초기화
            AppPreferences.getInstance().setNewAppLockPassword(getApplicationContext(), "");
      ...
      public void onResumeAppLock()
      {
            ...
            if (AppPreferences.getInstance().isThereAppLockPassword(getApplicationContext()))
            {
                  Log.d(TAG, "등록된 앱 비밀번호가 있습니다.");

                  if (AppPreferences.getInstance().isApplockScreenOptionEnabled(getApplicationContext()))
                  {
                        Log.d(TAG, "Applock screen on!");
                        // 2. 등록된 패스워드가 있다면 패스워드 입력 화면을 출력
                        if (AppParam.getInstance().isAppLockState)
                        {
                              ((TextView) findViewById(R.id.main_applock_desc)).setText("앱 비밀번호를 입력해주세요.");
                              ((ConstraintLayout) findViewById(R.id.main_applock_layout)).setVisibility(View.VISIBLE);
      ...
      public void initAppLockPassword()
      {
            ...
            ((TextView) findViewById(R.id.main_applock_password_1_val)).setText(AppParam.getInstance().appLockNum1);
            ((TextView) findViewById(R.id.main_applock_password_2_val)).setText(AppParam.getInstance().appLockNum2);
            ((TextView) findViewById(R.id.main_applock_password_3_val)).setText(AppParam.getInstance().appLockNum3);
            ((TextView) findViewById(R.id.main_applock_password_4_val)).setText(AppParam.getInstance().appLockNum4);
      }
      ...
      public void inputUnlockNum(int num)
      {
            ...
            {
                  // 앱 비밀번호 등록 과정
                  if (AppParam.getInstance().doesNeedToRegisterAppLockPassword)
                  {
                        // 비밀번호 등록 과정의 첫번째 입력
                        if (!AppParam.getInstance().doesNeedToVerifyAppLockPassword)
                        {
                              Log.d(TAG, "등록을 위한 앱 비밀번호를 입력했습니다.");
                              AppParam.getInstance().strFirstLockNum = AppParam.getInstance().appLockNum1 + AppParam.getInstance().appLockNum2 + AppParam.getInstance().appLockNum3 + AppParam.getInstance().appLockNum4;
                              AppParam.getInstance().doesNeedToVerifyAppLockPassword = true;
                              ((TextView) findViewById(R.id.main_applock_desc)).setText("확인을 위해 앱 비밀번호를 다시 입력해주세요.");
                              initAppLockPassword();
                        }
                        else // 비밀번호 등록 과정의 두번째 위한 (첫번째 입력과 일치하는지 비교하기 위함)
                        {
                              AppParam.getInstance().strSecondLockNum = AppParam.getInstance().appLockNum1 + AppParam.getInstance().appLockNum2 + AppParam.getInstance().appLockNum3 + AppParam.getInstance().appLockNum4;

                              // 두 비밀번호 비교
                              if (AppParam.getInstance().strFirstLockNum.equals(AppParam.getInstance().strSecondLockNum)) // 등록을 위해 입력한 두 비밀번호가 같을 때
                              {
                                    Log.d(TAG, "등록을 위한 앱 비밀번호의 확인용 비밀번호가 일치합니다.");

                                    AppPreferences.getInstance().setNewAppLockPassword(getApplicationContext(), AppParam.getInstance().strFirstLockNum);
                                    AppParam.getInstance().doesNeedToVerifyAppLockPassword = false;
                                    AppParam.getInstance().doesNeedToRegisterAppLockPassword = false;

                                    initAppLockPassword();

                                    // 잠금 화면 해제
                                    ((ConstraintLayout) findViewById(R.id.main_applock_layout)).setVisibility(View.GONE);
                              }
                              else // 등록을 위해 입력한 두 비밓번호가 다를 때
                              {
                                    AppParam.getInstance().doesNeedToVerifyAppLockPassword = false;
                  ...
                  else // 일반적인 앱 비밀번호 확인 과정
                  {
                        ...
                        if (AppPreferences.getInstance().isAppLockPasswordCorrect(getApplicationContext(), AppParam.getInstance().strFirstLockNum))
                        {
            ...
            switch (item.getItemId())
            {
                  ...
                  case R.id.menu_applock_screen:
                  {
                        ...
                        builder.setPositiveButton("네", new DialogInterface.OnClickListener()
                        {
                              ...
                              {
                                    updateLongTimeIdleHandler();
                                    Log.d(TAG, "Applock screen enabled state = " + AppPreferences.getInstance().isApplockScreenOptionEnabled(getApplicationContext()));
